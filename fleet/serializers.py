...
from fleet.models import (User, VehicleService, ServiceType,
                         SubService, Vehicle, VehiclePart)
...
class SubServiceSerializer(serializers.ModelSerializer):
        ...
        if service_type and subservice_name:
            if service_type.service_type == 'Regular Maintenance' and subservice_name not in ['Oil change', 'Fluid checks']:
                raise serializers.ValidationError(
                    "Invalid subservice_name for Regular Maintenance service_type.")
    ...
    def create(self, validated_data):
        ...
        service_type = SubService.objects.create(
...
class VehiclePartSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user.user_type != "mechanic":
            raise serializers.ValidationError(
                "Only mechanics can inspect and write the report on vehicle wellbeing.")
        return data

    def create(self, validated_data):
        # Extract the sub_service_id from validated_data
        vehicle_part_sub_service = validated_data.pop(
            'vehicle_part_sub_service', None)

        vehicle_part = VehiclePart.objects.create(
            vehicle_part_sub_service=vehicle_part_sub_service, **validated_data)
...
class VehicleServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id',  'vehicle_next_service_date',
                  'vehicle_previous_service_date', 'vehicle_part']

    def validate(self, data):
        request = self.context.get('request', None)
        vehicle_next_service_date = data.get('vehicle_next_service_date')
        vehicle_previous_service_date = data.get(
            'vehicle_previous_service_date')
        if request is None:
            raise serializers.ValidationError("Request object is invalid.")
        user = request.user
    ...
    def create(self, validated_data):
        ...
        vehicle_service = VehicleService.objects.create(
            vehicle_part=vehicle_part, **validated_data)
...
class VehicleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if vehicle_owner and vehicle_owner.user_type != 'vehicle_owner':
            raise serializers.ValidationError(
                "Vehicle owner must have user type 'vehicle_owner'.")
        print(vehicle_owner)
        return data
